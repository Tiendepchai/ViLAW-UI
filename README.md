# ĐATT Chat UI (repo riêng)

Repo này cung cấp **UI hỏi–đáp kiểu ChatGPT** (có sidebar history, chọn cuộc hội thoại) và **lưu lịch sử hội thoại vào JSON**.

Nó **không chứa logic RAG**; chỉ:
- Gọi API của dự án gốc (qua `PROJECT_API_BASE`).
- Lưu/đọc hội thoại từ `data/conversations.json`.
...
- Repo này: `datt-chat-ui`
- Repo dự án gốc: giữ nguyên.
